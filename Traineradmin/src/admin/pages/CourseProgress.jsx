...
const CourseProgress = () => {
  ...
  // Resolve course title from slug
  useEffect(() => {
    const titleMap = {
      ...
      <div className="max-w-7xl mx-auto px-8 mt-6">
        <div className="relative rounded-[2rem] overflow-hidden bg-gradient-to-r from-indigo-950 via-slate-900 to-indigo-950 text-white p-8 md:p-12 flex flex-col md:flex-row items-center justify-between gap-8">
          ...
          <div className="relative z-10 flex flex-col md:flex-row items-center gap-8 text-left">
            {/* Course icon */}
            <div className="w-28 h-28 rounded-2xl bg-indigo-900/60 border border-white/10 p-3 flex flex-col items-center justify-center shadow-2xl shrink-0">
              <img
              ...
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-slate-100 text-xs font-semibold text-slate-600">
                  {filtered.map((s) => {
                    ...
                    return (
                      <tr key={s.student_id} className="hover:bg-slate-50/60 transition-colors group">

                        {/* Student */}
                        <td className="p-4 pl-6">
                          <div className="flex items-center gap-3">
                            {/* Avatar initials */}
                            <div className="w-9 h-9 rounded-full bg-gradient-to-br from-indigo-400 to-purple-600 flex items-center justify-center text-white text-xs font-black shrink-0 shadow-sm">
                        ...
                        <td className="p-4">
                          ...
                          {s.enrollment_date && (
                            <p className="text-[9px] text-slate-400 mt-0.5 flex items-center gap-1">
                              <Calendar size={9} /> Enrolled: {formatDate(s.enrollment_date)}
                            </p>
                          )}
                        </td>
                        ...
                        <td className="p-4 text-center">
                          <span className="inline-flex items-center justify-center w-9 h-9 rounded-full bg-emerald-50 border border-emerald-100 text-emerald-700 font-black text-sm">
                            {s.completed_assignments ?? 0}
                          </span>
                          <p className="text-[9px] text-slate-400 mt-0.5">of {s.total_assignments ?? totalAssignments}</p>
                        </td>
                        ...
                        <td className="p-4 min-w-[160px]">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="font-extrabold text-slate-700 text-xs w-10 text-right shrink-0">{pct}%</span>
                          </div>
                        </td>
                        ...
                        <td className="p-4">
                          {s.last_activity_date ? (
                            <div className="flex items-center gap-1.5 text-slate-500">
                              ...
                              <div>
                                <p>{formatDate(s.last_activity_date)}</p>
                                <p className="text-[9px] text-slate-400">
                                  {new Date(s.last_activity_date).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' })}
                                </p>
                              </div>
                            </div>
                          ) : (
                        ...
                        {/* Status Badge */}
                        <td className="p-4">
                          <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[10px] font-black uppercase tracking-wider ${cfg.badge}`}>
                        ...
                        {/* Action menu */}
                        <td className="p-4">
                          <button className="p-1.5 rounded-lg hover:bg-slate-100 text-slate-400 transition-colors opacity-0 group-hover:opacity-100">
