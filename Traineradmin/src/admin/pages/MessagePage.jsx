        ...
        <div className="flex items-center gap-3">
          <div className="relative">
            ...
            <input
              ...
              className="pl-10 pr-4 py-2.5 rounded-xl border border-slate-200 bg-white shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 text-sm w-full md:w-64 transition-all"
            />
          </div>
          <button
            onClick={fetchMessages}
            disabled={loading}
            className="p-2.5 bg-white border border-slate-200 rounded-xl text-slate-600 hover:bg-slate-50 hover:text-indigo-600 shadow-sm transition-all active:scale-95 disabled:opacity-50"
          >
          ...
          {filteredMessages.map((msg) => (
            ...
            >
              ...
              <div className="flex items-start justify-between mb-4">
                <div className="flex items-center gap-3">
                  <div className="w-12 h-12 bg-gradient-to-br from-indigo-100 to-purple-100 rounded-2xl flex items-center justify-center text-indigo-700 font-black text-lg shadow-inner">
              ...
              <div className="space-y-2.5 mb-5 bg-slate-50 p-4 rounded-2xl border border-slate-100/50">
                <div className="flex items-center gap-2.5 text-sm text-slate-600">
                  ...
                  <a href={`mailto:${msg.email}`} className="hover:text-indigo-600 transition-colors truncate">
                    {msg.email}
                  </a>
                </div>
                {msg.phone && (
                  <div className="flex items-center gap-2.5 text-sm text-slate-600">
                    ...
                    <a href={`tel:${msg.phone}`} className="hover:text-indigo-600 transition-colors">
                      {msg.phone}
                    </a>
                  </div>
                )}
                {msg.course && (
                  <div className="flex items-center gap-2.5 text-sm text-slate-600">
                    ...
                    <span className="font-semibold text-indigo-700 bg-indigo-100/50 px-2 py-0.5 rounded-lg truncate">
                      {msg.course}
                    </span>
                  </div>
                )}
